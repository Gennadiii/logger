...
function getArgsToLog(params: getArgsToLogInterface): string {
  ...
  try {
    return removePasswordFromLogs(logArgsLambda(args)); // diff
  } catch (err) {
...
// <diff
function removePasswordFromLogs(args: unknown): string {
  ...
  return new Proxy(obj, {
    get(component, method) {
      ...
      return (...args) => {
        ...
        const argsToLog = isPasswordInput(element) ? ["***"] : args; // diff
...
// <diff
function isPasswordInput(elementName: string): boolean {
