...
export class SomeVeryLongNameForLoginService {
  ...
  async using(params: usingInterface): Promise<void> {
    ...
    await this.loginPage.getModeButton('admin').click(); // diff
    await this.loginPage.input.emailInputField.enterText(email);
    await this.loginPage.input.passwordInputField.enterText(password);
